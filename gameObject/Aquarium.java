...
public class Aquarium implements Serializable {
	...
	private LinkedList<Alien> listAlien = new LinkedList<Alien>();
	private Snail garry;
	private int egg;
	private int money;
...
//		System.out.println("Create Guppy");
		System.out.println("±¸ÇÇ "+listGuppy.getSize());
	}
	...
	public Point getClosestAlien(Point p) {
		if (!listAlien.isEmpty()) {
			Point temp = new Point(listAlien.get(0).getX(), listAlien.get(0).getY());
			for (int i = 0; i < listAlien.getSize(); i++) {
				if (p.findDistance(listAlien.get(i)) < p.findDistance(temp)) {
					temp.setX(listAlien.get(i).getX());
					temp.setY(listAlien.get(i).getY());
				}
			}
			return temp;
		}
	...
	/**
	 * method to move alien in aquarium.
	 */
	public void moveAlien() {
		Point locAlien = new Point();
		...
		for (int i = 0; i < listAlien.getSize(); i++) {
			if (listAlien.get(i).AlienisTimeToDie(listAlien.get(i).getClick())) {
				...
				if (!listGuppy.isEmpty() && !listPiranha.isEmpty()) {
					...
					locAlien.setX(listAlien.get(i).getX());
					locAlien.setY(listAlien.get(i).getY());

					Point temp = getClosestGuppy(locAlien);
					...
					for (int idx = 0; idx < listGuppy.getSize(); idx++) {
						if (listGuppy.get(idx).getX() == closest.getX()
							...
							listAlien.get(i).moveToEat(listGuppy, listGuppy.get(idx), getCurrentTime(), eat);
